...
def audio_processing_thread():
    ...
    for key, value in person_stuff.items():
        print(f"  {key.capitalize()}: {value}")
def main():
    global running, saved_face_ratios, start_time, status_message, current_color
   # listening thread beginning
    listener_thread = threading.Thread(target=always_listening, daemon=True)
    listener_thread.start()
    # thread processing for audio - begin
        ...
        while webcam.isOpened():
            ...
            if results.multi_face_landmarks:
                for face_landmarks in results.multi_face_landmarks:
                    ...
                    #box will be position to the right of the face
                    box_x = nose_x + 50
                    box_y = nose_y - 100
                    overlay = frame.copy()
                    box_width = 250
